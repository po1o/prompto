...
// VimVariant is the prompt as it would render under one vim mode.
type VimVariant struct {
	Primary string
	RPrompt string
}

// VimVariants renders the prompt under each of the given vim modes, reusing the
// segment state of the render that just finished. The shell caches the result
// and switches modes by swapping strings, instead of spawning a process and
// making a round trip to change one segment's text.
//
// It must only be called on a completed render generation. It goes through
// PrimaryRepaint, which marks the canonical vim segment authoritative and so
// makes later async vim results be dropped (see mergeSegmentResult); a cold
// start still needs that merge, and calling this mid-render would lose it.
//
// The engine is left rendering the mode it was called under: the deferred
// repaint puts the vim segment back, so a caller reading StreamingRPrompt
// afterwards sees the same prompt it would have seen without this call.
func (e *Engine) VimVariants(modes ...string) map[string]VimVariant {
	if len(modes) == 0 || e.Env == nil {
		return nil
	}
	...
	current := flags.VimMode

	defer func() {
		flags.VimMode = current
		_ = e.PrimaryRepaint()
	}()

	variants := make(map[string]VimVariant, len(modes))

	for _, mode := range modes {
		flags.VimMode = mode
		variants[mode] = VimVariant{
			Primary: e.PrimaryRepaint(),
			RPrompt: e.StreamingRPrompt(),
		}
	}
