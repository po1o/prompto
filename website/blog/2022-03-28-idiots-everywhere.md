...
of using an old and new version of Prompto at the same time. Who does that you might ask? Well, it can be because you're
...
can use `prompto config migrate` manually), or when the current config version doesn't match the required config
version. _Simple, right?_ And this works according to expectations when updating Prompto. The problem starts when you
have a shared config, and use multiple versions of Prompto on the same machine.

Imagine working using multiple shells, one is WSL Ubuntu with a version of Prompto that's still on version 1
...
and due to the logic, Prompto sees that `2 != 1` and **the migration is triggered for version 1**. The migration as such
is non-breaking, but Prompto doesn't know the `template` property on `segment` in this case. The end result of our
...
😱 the template is gone! Nothing to worry about just yet, Prompto will use the default template so you will still see
...
at once**, so you only migrate once and there's no "old" version of Prompto left to trigger this unwanted side-effect.
...
[fix-version]: https://github.com/po1o/prompto/releases/tag/v7.52.1
