...
What a week it’s been! Prompto turned 6 years old and we dropped a ton of stuff!

{/* truncate */}
## Swag

After seeing Scott Hanselman wear his oh my zsh shirt while we got to talk about
Prompto on [Windows Wednesdays][windows-wednesdays], I got the sudden urge to expand his wardrobe.

<iframe
  className="youtube"
  src="https://www.youtube.com/embed/uO_F5W2LbSk"
  title="Windows Wednesdays: Prompto"
...
Consider this to be **the first artist series** for Prompto, a way to also highlight some talented
...
Something I've been sleeping on for a long time, Prompto finally has a new CLI interface.
...
You can now interact with Prompto via an interface powered by [Cobra][cobra]. Not only does this
...
experience. So, as of [7.42.0][7.42.0] Prompto can initialize your prompt without specifying the
`--config` flag. Because we have no way to know how you installed Prompto, and if you have themes
...
prompto config export --output=~/.mytheme.omp.json
...
Thanks to [Jed Willick][jed], who's already been on a roll for a while adding segments to Prompto,
...
- The poshgit prompt is now disabled when using Prompto
