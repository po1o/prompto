...
even more straightforward. As Prompto is a prompt generating utility, the `prompt` command
...
```powershell
prompto prompt init pwsh | Invoke-Expression
```
...
To debug Prompto, you can simply use the `debug` command. It's smart enough to know your config
...
Looking at Prompto's architecture, the different prompts (transient, secondary, ...) were moving closer to the model
...
Not something that impacts the end user directly, but maybe interesting to know is that Prompto is now
