...
In the beginning, all we had was the [PowerShell Module][module]. It's the _origin of Prompto_
...
A fews weeks ago I started a [giveaway][giveaway] to celebrate the 6 year anniversary of Prompto.
...
In the early days, Prompto was forcibly setting PowerShell to UTF-8 due to issues with fonts and paths.
...
set the shell to UTF-8 in the scope of initializing Prompto.
...
If you haven't seen [Nu shell][nu] yet, it's a new kid on the block. Prompto now supports it just
like we support any other popular shell (bash, zsh, fish, etc). This means we can keep adding functionality
without the need for user interaction apart from updating Prompto.
...
Wait, what? Yes, you read that right. Prompto now supports **cross segment template properties**.
This means you can use one segment's properties in another segment's template. How? Prompto
...
your config does not contain a git segment as Prompto only populates the properties when it needs to.
:::

### Other

- `prompto debug` now measures the run time correctly and can log startup logic
...
[module]: https://www.powershellgallery.com/packages/prompto/7.85.2
