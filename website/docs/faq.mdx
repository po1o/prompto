...
Before validating anything, make sure you're on the [latest version][latest] of Prompto and your terminal and shell are up-to-date.

## My prompt is broken after upgrading to PowerShell 7.4

PowerShell 7.4 has an [issue][pwsh-20733] in encoding which affects the rendering of Prompto.
...
```bash
prompto disable notice
```

If you want to enable it again, run:

```bash
prompto enable notice
```
...
You can use the `prompto debug` functionality to see where Prompto spends its time.
...
You can use Prompto's built-in debug functionality to identify slow segments.
...
To solely rely on Prompto to set the prompt, you can configure the following setting to hide it.
Make sure to add this **before** initializing Prompto.
...
https://prompto.dev/docs/faq#powershell-running-in-constrainedlanguage-mode
```
...
To remove the message after manual configuration, you can add the following to your `$PROFILE` before importing Prompto:

```powershell
$env:PROMPTO_CONSTRAINED_LANGUAGE = 1
```

## PowerShell: The term '.../prompto.exe' is not recognized as a name of a cmdlet...

For example:

```
&: The term 'C:/Users/TommiGr├╢nlund/.prompto/prompto.exe' is not recognized as a name of a cmdlet, function, script file, or executable program.
Check the spelling of the name, or if a path was included, verify that the path is correct and try again.
```

The issue is that PowerShell on Windows doesn't yet default to UTF-8. Resolve the issue by setting the shell to UTF-8 in the scope of initializing Prompto.

:::info Why it matters?
If the location contains non-ASCII characters, non-UTF-8 PowerShell may provide a wrong path to Prompto, which can break the initialization.
...
To do so, edit your `$PROFILE`, find the line that initializes Prompto (as highlighted below), and change it to the following:
...
This issue occurs when you're using plain zsh in combination with Prompto.
...
This is most likely caused by two Prompto init lines in your `.zshrc`, remove one of them. See [here][zsh-init].

## Fish: Display current bind (Vim) mode

By default, Prompto will not re-render the prompt (i.e., generate a new prompt) until a new command is run, so you should
...
To have prompto update the directory context as the directory history is navigated, use a function that calls `omp_repaint_prompt` when the `$PWD` variable changes:
...
This can cause the prompt to display unknown characters. There's a built-in migration in Prompto to fix this.

To migrate, run the following command:

```bash
prompto config migrate glyphs --write
```
...
[new-issue]: https://github.com/po1o/prompto/issues/new/choose
[latest]: https://github.com/po1o/prompto/releases/latest
...
[zsh-init]: https://github.com/po1o/prompto/discussions/3462#discussioncomment-5155790
