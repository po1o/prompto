...
| `source`              |      `string`       |  `cli`  | <ul><li>`cli`: fetch the information using the git CLI</li><li>`pwsh`: fetch the information from the [prompto-git][poshgit] PowerShell Module</li></ul>                                                                                                                                                                                 |
...
If you want to display the default [prompto-git][poshgit] output, **do not** use this segment
but add the following snippet after initializing Prompto in your `$PROFILE`:
...
<Config
  data={{
    template:
      "{{ if .Env.PROMPTO_GIT_STRING }} {{ .Env.PROMPTO_GIT_STRING }} {{ end }}",
  }}
/>

[git]: https://git-scm.com/
[poshgit]: https://github.com/dahlbyk/prompto-git
