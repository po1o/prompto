...
The segment reads session information from the `PROMPTO_CLAUDE_STATUS` environment variable, which is automatically set when using the `prompto claude` command. The data includes:
