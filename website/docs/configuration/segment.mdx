        ...
        segments: [
          {
            ...
            background: "#61AFEF",
            template: " {{ .Path }} ",
            include_folders: ["/Users/prompto/Projects"],
          },
        ],
      },
    ],
...
| `leading_powerline_symbol` | `string`     |         | character to use at the start of the segment when `"style": "powerline"`. By default, Prompto uses an ANSI hack to invert the `powerline_symbol`, which provides the best alignment but may not work in all terminals. If you see black artifacts at segment starts, set this to the corresponding opening glyph (e.g., `\uE0D6` when using `\uE0B0`, or `\uE0D7` when using `\uE0B1`) |
...
| `min_width`                | `int`        |   `0`   | if the terminal width is smaller than this value, the segment will be hidden. For your terminal width, see `prompto get width`. Defaults to `0` (disable)                                                                                                                                                                                                                              |
| `max_width`                | `int`        |   `0`   | if the terminal width exceeds this value, the segment will be hidden. For your terminal width, see `prompto get width`. Defaults to `0` (disable)                                                                                                                                                                                                                                      |
...
Unfortunately, it's not possible for Prompto to know the final prompt length since the string expansion is done
...
<Config
  data={{
    exclude_folders: ["/Users/prompto/Projects"],
  }}
/>
...
name. The following will match `/Users/prompto/Projects/Foo` but not `/home/Users/prompto/Projects/Foo`.
...
<Config
  data={{
    include_folders: ["/Users/prompto/Projects/.*"],
    exclude_folders: ["/Users/prompto/Projects/secret-project.*"],
  }}
/>

:::info

- Prompto will accept both `/` and `\` as path separators for a folder and will match regardless of which
...
a diamond segment, only if the environment variable `PROMPTO_ENV` is not empty.
...
<Config
  data={{
    ...
    foreground: "#ffffff",
    background: "#d53c14",
    template: "{{ if .Env.PROMPTO_ENV }} \uf8c5 {{ .Env.PROMPTO_ENV }} {{ end }}",
  }}
/>
...
using a conditional template. In this case you can use the `prompto toggle <type>` command to toggle the
...
To list the currently toggled segments, use `prompto get toggles`.
