...
Prompto renders your prompt based on the definition of _blocks_ (like Lego) which contain one or more _segments_.
...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/JanDeDobbeleer/prompto/main/themes/schema.json
final_space: true
version: 4
blocks:
  - type: prompt
...
To set this configuration in combination with an Prompto [executable][releases], use the `--config` flag to
...
```bash
prompto print primary --config sample.json --shell uni
```
...
| `daemon_timeout`            | `int`            | `100`   | timeout in milliseconds before daemon render returns the initial batch and continues streaming updates asynchronously                                                                                                                                                |
| `daemon_idle_timeout`       | `string`         | `5`     | daemon auto-stop timeout in minutes after all tracked shell sessions exit. Set to `none` to disable auto-stop                                                                                                                                                             |
...
As mentioned above, Prompto configurations can utilize JSON Schema to validate their contents. Configurations should include a link to
...
Prompto supports three file formats for configurations: `json`, `yaml`, and `toml`.
...
[releases]: https://github.com/po1o/prompto/releases/latest
[font]: /docs/installation/fonts
[schema]: https://github.com/po1o/prompto/blob/main/themes/schema.json
