...
Open your prompto repo inside GitKraken and right click the commit you want to reword in the graph overview.
...
This means the main branch of prompto contains commits your branch does not (could be your main branch, or the branch you created to work on).
To remedy this, we need to rebase (add the new commits of prompto's main branch underneath your new commits) so the pull request can get merged.

The first thing to do is to add the prompto codebase as a remote to your local git repository. By default, your fork is a standalone copy
of prompto with its own remote on GitHub that's not connected to the prompto codebase. Forks and Pull Requests are a feature GitHub introduced
...
`jandedobbeleer/prompto` and name it `upstream`. GitKraken will fetch the remote and you will see all branches underneath `upstream` as
...
```bash
git remote add upstream git@github.com:JanDeDobbeleer/prompto.git
git fetch upstream
```
