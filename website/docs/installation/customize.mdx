...
The standard initialization sets Prompto' default, built-in theme.

To set a new configuration or theme you need to change the `--config` option of the `prompto init <shell>`
...
- a path to a local configuration file
  ...
  ```powershell
  --config 'C:/Users/Prompto/myconfig.omp.json'
  ```
...
- a URL pointing to a remote config
  ...
  ```powershell
  --config 'https://raw.githubusercontent.com/JanDeDobbeleer/prompto/main/themes/jandedobbeleer.omp.json'
  ```
...
Adjust the Prompto init line in `~/.bashrc` (could be `~/.profile` or `~/.bash_profile` depending
on your environment) by adding the `--config` flag with the location of your configuration.

```bash
eval "$(prompto init bash --config ~/jandedobbeleer.omp.json)"
```
...
Adjust the Prompto init line in `prompto.lua` by adding the `--config` flag with the location
of your configuration.

```lua title="prompto.lua"
load(io.popen('prompto init cmd --config C:/Users/Prompto/jandedobbeleer.omp.json'):read("*a"))()
```
...
Adjust the Prompto init line in `~/.elvish/rc.elv` by adding the `--config` flag with the location
of your configuration.

```bash
eval (prompto init elvish --config ~/jandedobbeleer.omp.json)
```
...
Adjust the Prompto init line in `~/.config/fish/config.fish` by adding the `--config` flag with
the location of your configuration.

```bash
prompto init fish --config ~/jandedobbeleer.omp.json | source
```
...
:::caution
Prompto requires Nushell `v0.104.0` or higher.
:::

Adjust the Prompto init line in the Nushell config file (`$nu.config-path`) by adding the `--config` flag
with the location of your configuration.

```bash
prompto init nu --config ~/jandedobbeleer.omp.json
```

</TabItem>
<TabItem value="powershell">

Adjust the Prompto init line in your `$PROFILE` by adding the `--config` flag with the location
of your configuration.

```powershell
prompto init pwsh --config ~/jandedobbeleer.omp.json | Invoke-Expression
```
...
:::info
When the above command gives an error, make sure to create the profile first and add the `prompto init` above.
...
Adjust the Prompto init line in `~/.xonshrc` by adding the `--config` flag with the location of your configuration.

```bash
execx($(prompto init xonsh --config ~/jandedobbeleer.omp.json))
```
...
Adjust the Prompto init line in `~/.zshrc` by adding the `--config` flag with the location of your configuration.

```bash
eval "$(prompto init zsh --config ~/jandedobbeleer.omp.json)"
```
...
:::tip wsl
When using prompto in Windows and the WSL, know that you can **share your theme with the WSL** by pointing to a theme in your
...
```bash
prompto config export --config jandedobbeleer --output ~/.mytheme.omp.json
```
...
```bash
prompto enable reload
```

You can disable live reload with:

```bash
prompto disable reload
```
