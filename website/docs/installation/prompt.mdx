...
:::tip
If you have no idea which shell you're currently using, Prompto has a utility switch that can tell that to you.
:::
...
To enable Claude Code integration with Prompto, configure Claude Code to use Prompto as the statusline command.
...
Integrating Prompto with Clink is easy: create a new file called prompto.lua in your Clink
scripts directory (run `clink info` inside cmd to find that file's location).

```lua title="prompto.lua"
load(io.popen('prompto init cmd'):read("*a"))()
```

Once added, restart cmd for the changes to take effect.

:::info
Clink has builtin support for Prompto. It allows you to set the prompt using the `clink` command.

```bash
clink config prompt use prompto
```

To set the configuration file, use the following command:

```bash
clink set prompto.theme <path>
```
:::
...
```powershell
prompto init pwsh --eval | Invoke-Expression
```
...
Note this will still load Prompto for [iTerm2][iterm2] or any other modern day macOS terminal that supports ANSI characters.
:::
